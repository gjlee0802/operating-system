...
### 1. 프로세스가 fork() 를 사용하여 새로운 프로세스를 생성할 때, 다음 중 부모 프로세스와 자식 프로세스 사이에 공유되는 상태는 무엇인가? (P 22-23 First)
~~~
(a) Stack
(b) Heap
...
|----------------------|------------------------|----------------------------------|
| **Stack**            | ❌ 복사됨 (독립적)     | 함수 호출, 지역 변수 저장       |
| **Heap**             | ❌ 복사됨 (독립적)     | 동적 메모리 (`malloc`, `new` 등) |
| **Data**             | ❌ 복사됨 (독립적)     | 전역 변수, static 변수          |
| **Code/Text**        | ✅ 대부분 공유 (읽기 전용) | 실행 코드. 보통 읽기 전용으로 공유 |
| **Shared Memory**    | ✅ 공유됨              | `shmget`, `mmap` 등을 통해 명시적으로 공유 |
| **Registers**        | ❌ 복사됨              | 레지스터 상태도 복사되지만 서로 영향 없음 |
